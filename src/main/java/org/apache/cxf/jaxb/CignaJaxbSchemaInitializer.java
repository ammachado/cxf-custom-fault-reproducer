 ...
 */
class CignaJaxbSchemaInitializer extends ServiceModelVisitor {
    private static final Logger LOG = LogUtils.getLogger(CignaJaxbSchemaInitializer.class);

    private SchemaCollection schemas;
    private JAXBContextProxy context;
    private final boolean qualifiedSchemas;

    CignaJaxbSchemaInitializer(ServiceInfo serviceInfo,
    ...
    private void buildExceptionType(MessagePartInfo part, Class<?> cls) {
        ...
        XmlAccessType accessType = CignaJaxBUtils.getXmlAccessType(cls);
//
        for (Field f : CignaJaxBUtils.getFields(cls, accessType)) {
            //map field
            Type type = CignaJaxBUtils.getFieldType(f);
        ...
        for (Method m : CignaJaxBUtils.getGetters(cls, accessType)) {
            //map method
            Type type = CignaJaxBUtils.getMethodReturnType(m);
            // we want to return the right type for collections so if we get null
            // from the return type we check if it's ParameterizedType and get the
            // generic return type.
    ...
    private void buildGenericSeq(XmlSchema schema, XmlSchemaComplexType generics, Class<?> genericsClass) {
        ...
        XmlAccessType accessType = CignaJaxBUtils.getXmlAccessType(genericsClass);

        for (Field f : CignaJaxBUtils.getFields(genericsClass, accessType)) {
        ...
        for (Method genericMethod : CignaJaxBUtils.getGetters(genericsClass, accessType)) {
