...
class CignaJAXBContextInitializer extends ServiceModelVisitor {
    private static final Logger LOG = LogUtils.getL7dLogger(CignaJAXBContextInitializer.class);
    ...
    CignaJAXBContextInitializer(Bus bus, ServiceInfo serviceInfo,
                           Set<Class<?>> classes,
    ...
    private void checkForAdapter(Class<?> clazz, Annotation[] anns) {
        if (anns != null) {
            for (Annotation a : anns) {
                if (XmlJavaTypeAdapter.class.isAssignableFrom(a.annotationType())) {
                    Type t = CignaJaxBUtils.getTypeFromXmlAdapter((XmlJavaTypeAdapter)a);
                    if (t != null) {
                        addType(t);
                    }
                }
        ...
        if (xjta != null) {
            Type t = CignaJaxBUtils.getTypeFromXmlAdapter(xjta);
            if (t != null) {
                addType(t);
            }
        }
        if (clazz.getPackage() != null) {
            ...
            if (adapt != null) {
                ...
                for (XmlJavaTypeAdapter a : adapt.value()) {
                    Type t = CignaJaxBUtils.getTypeFromXmlAdapter(a);
                    if (t != null) {
                        addType(t);
                    }
                }
        ...
        if (accessType != XmlAccessType.PROPERTY) {   // only look for fields if we are instructed to
            ...
            for (Field f : fields) {
                if (isFieldAccepted(f, accessType)) {
                    XmlJavaTypeAdapter xjta = CignaJaxBUtils.getFieldXJTA(f);
                    if (xjta != null) {
                        Type t = CignaJaxBUtils.getTypeFromXmlAdapter(xjta);
                        if (t != null) {
                            addType(t);
        ...
        if (accessType != XmlAccessType.FIELD) {   // only look for methods if we are instructed to
            ...
            for (Method m : methods) {
                if (isMethodAccepted(m, accessType)) {
                    XmlJavaTypeAdapter xjta = CignaJaxBUtils.getMethodXJTA(m);
                    if (xjta != null) {
                        Type t = CignaJaxBUtils.getTypeFromXmlAdapter(xjta);
                        if (t != null) {
                            addType(t);
    ...
    private static Object createTypeReference(QName n, Class<?> cls) {
        ...
        try {
            refClass = ClassLoaderUtils.loadClass("org.glassfish.jaxb.runtime.api.TypeReference",
                                                  CignaJAXBContextInitializer.class);
        } catch (Throwable ex) {
            //ignore
        }
        if (refClass != null) {
